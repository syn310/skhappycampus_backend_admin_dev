const models = require('../../model/menuBp/Menu');
...
exports.update = (req,res) => {
  const bpMenuId = req.params.bpMenuId || '';
  ...
  //console.log("menuInfo", req.body.menuInfo)

  if(!bpMenuId.length){
    return res.status(400).json({error:systemMessage.search.incorrectKey + "bpMenuId" , req:bpMenuId});
  }
  if(!menuName.length){
  ...
  //const depth = url.length > 0 && subUrl.length > 0 && parent.length > 0 ? 2 : 1; 

  ////console.log("bpMenuId",bpMenuId, depth)
  const newDate = new Date()
  const time = newDate.toFormat('YYYY-MM-DD HH24:MI:SS');

  models.MenuBp.update({
      menuName: menuName,
      url: url,
